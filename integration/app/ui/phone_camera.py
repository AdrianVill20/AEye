...
def pick_cameras(names=None):
    ...
    phones = [i for i, n in enumerate(names) if 'iriun' in n.lower()][::-1]   # phone cameras
...
def populate_camera_combo(combo, default_index=None):
    # Fill the dropdown with camera names and select the face camera.
    combo.clear()
    ...
    if default_index is None:
        default_index = pick_cameras(names)[0]   # pick it by itself
...
if __name__ == '__main__':      # test: python -m ui.phone_camera
    this_pc = ['Iriun Webcam #2', 'Iriun Webcam', 'A4tech FHD 1080P PC Camera', 'OBS Virtual Camera']
    assert pick_cameras(this_pc) == (2, 1)                               # webcam face, phone side
    assert pick_cameras(['Integrated Camera', 'USB Camera']) == (0, 1)   # two webcams
    assert pick_cameras(['OBS Virtual Camera', 'HD Webcam']) == (1, 1)   # one webcam
    assert pick_cameras([]) == (0, 1)                                    # no names
